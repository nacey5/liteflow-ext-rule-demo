...
public class ChainExecute implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        LiteflowResponse response = flowExecutor.execute2Resp("calculatePercentage", null, CaculateContext.class);
        CaculateContext context = response.getFirstContextBean();
        if (response.isSuccess()){
            log.info("执行成功，最终选择的渠道是{}", context.isSuccess());
        }else{
